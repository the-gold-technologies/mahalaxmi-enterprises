...
export default function AboutMahalaxmiContent() {
  ...
  const cmsStory = pages["about-us"]?.MahalaxmiStory;
  const cmsOverview = pages["about-us"]?.HpclOverview;
        ...
        {cmsOverview?.description && (
          <div className="mt-8 p-6 bg-slate-50 border border-slate-200 rounded-xl">
            <h3 className="text-lg font-bold text-[#002b5c] mb-2">
              {cmsOverview.title || "About Hindustan Petroleum Corporation Limited (HPCL)"}
            </h3>
            <p className="text-sm text-gray-700 leading-relaxed mb-3">
              {cmsOverview.description}
            </p>
            {Array.isArray(cmsOverview.bullets) && cmsOverview.bullets.length > 0 && (
              <ul className="space-y-1.5 text-xs text-gray-600 list-disc list-inside">
                {cmsOverview.bullets.map((b: string, idx: number) => (
                  <li key={idx}>{b}</li>
                ))}
              </ul>
            )}
